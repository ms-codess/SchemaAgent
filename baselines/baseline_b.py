...
Top-k relevant tables are retrieved and injected into the prompt.
Claude generates SQL in one shot (no retry on error).
Expected ~52% EX on BIRD Mini-Dev — proves RAG alone helps.
"""
import logging
import os
import sys
...
RESULTS_PATH = "results/baseline_b.json"
...
def run(
    ...
    """
    Run Baseline B over the BIRD Mini-Dev set.
    ...
    # Pre-index any databases that haven't been indexed yet
    indexed_dbs: set = set()

    results: List[Dict[str, Any]] = []
    correct = 0

    for item in tqdm(questions, desc="Baseline B"):
        db_id = item["db_id"]
        ...
        # Fall back to full schema if RAG returns nothing
        if not schema_context.strip():
            logger.warning("RAG returned empty context for db=%s, falling back to full schema", db_id)
    ...
    logger.info("Baseline B — EX: %.2f%% (%d/%d)", ex * 100, correct, total)
